...
def _task_dto(row: Task) -> TaskDTO:
    return TaskDTO(
        ...
        estimated_remaining_minutes=row.estimated_remaining_minutes,
...
class CoreStore:
    ...
    def list_tasks(
        ...
        *,
        ...
        if inbox_only:
            statement = statement.where(Task.estimated_remaining_minutes.is_(None), Task.completed_at.is_(None))
    ...
    def update_task(session: Session, task_id: UUID, changes: TaskUpdate) -> TaskDTO:
        ...
        row.estimated_remaining_minutes = merged.estimated_remaining_minutes
    ...
    def adjust_remaining(session: Session, task_id: UUID, minutes: int | None) -> TaskDTO:
        ...
        return CoreStore.update_task(session, task_id, TaskUpdate(estimated_remaining_minutes=minutes))

    @staticmethod
    def defer_task(session: Session, task_id: UUID, earliest_start_at: datetime) -> TaskDTO:
        return CoreStore.update_task(session, task_id, TaskUpdate(earliest_start_at=as_utc(earliest_start_at)))
    ...
    def today(session: Session, now: datetime | None = None) -> TodayDTO:
        ...
        return TodayDTO(
            ...
            tasks=tasks,
            events=events,
            inbox_count=sum(1 for task in tasks if task.estimated_remaining_minutes is None),
        )
